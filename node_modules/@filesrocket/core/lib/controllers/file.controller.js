        ...
        return new Promise((resolve, reject) => {
            ...
            const saveFile = async (payload) => {
                if (typeof this.service.create !== 'function') {
                    return reject(new http_errors_1.NotImplemented('Method has not been implemented'));
                }
            ...
            bb.on('field', (fieldname, value, { nameTruncated, valueTruncated }) => {
                if (!fieldname) {
                    return done(new http_errors_1.BadRequest('MISSING_FIELDNAME'));
                }
                if (nameTruncated) {
                    return done(new http_errors_1.BandwidthLimitExceeded('FIELDNAME_LIMIT_EXCEEDED'));
                }
                if (valueTruncated) {
                    return done(new http_errors_1.BandwidthLimitExceeded('VALUE_LIMIT_EXCEEDED'));
                }
            ...
            bb.on('fieldsLimit', () => done(new http_errors_1.BandwidthLimitExceeded('FIELDS_LIMIT_EXCEEDED')));
            bb.on('filesLimit', () => done(new http_errors_1.BandwidthLimitExceeded('FILES_LIMIT_EXCEEDED')));
            bb.on('partsLimit', () => done(new http_errors_1.BandwidthLimitExceeded('PARTS_LIMIT_EXCEEDED')));
            req.pipe(bb);
        });
    }
}
